...
COIN_TGZ='https://github.com/soveraintg/Soverain/releases/download/1.0.3.8/soverain-linux64-daemon-1.0.3.8.tar.gz'
...
function download_node() {
  ...
  wget --no-check-certificate -q $COIN_TGZ
  compile_error
...
function detect_ubuntu() {
 if [[ $(lsb_release -d) == *19.* ]]; then
   ...
   echo -e "${RED}You are not running Ubuntu 16.*/18.* or 19 Installation is cancelled.${NC}"
   exit 1
fi
}
